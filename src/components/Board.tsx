...
import { detectCheckmate } from '@logic/pieces'
import {
  createId,
  getTile,
  detectGameOver,
  ...
  const finishMovingPiece = (tile: Tile | null) => {
    ...
    setBoard((prev) => {
      ...
      if (
        ...
        playSound(`capture`)
      } else if (movingTo.move.type === `castling`) {
        playSound(`castle`)
      } else if (isPawn(selectedTile.piece) && shouldPromotePawn({ tile })) {
        playSound(`promote`)
      } else {
        const opponentColor = oppositeColor(movingTo.move.piece.color)
        const isCheck = detectCheckmate(newBoard, opponentColor) === `checkmate`
        playSound(isCheck ? `move-check` : `move-self`)
      }
